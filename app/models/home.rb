class Home < ApplicationRecord
  ...
  def self.calendar
    Reservation.select("reservations.id, reservations.date, CONCAT(customers.firstname || ' ' || customers.lastname) AS name, products.product_name AS productname")
        .joins(:product).joins(:customer)
  end
